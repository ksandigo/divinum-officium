...
sub getrank {
  ...
  my @srank = ();
	...
	if ($transfertemp && $transfertemp !~ /tempora/i) { $transfertemp = subdirname('Sancti', $version) . "$transfertemp"; }	# unless Tempora, it's a sancti transfer

	my $transfer = get_transfer($year, $version, $sday);
	
	$hymncontract = get_transfer($year, $version, "Hy$sday");
	
	if ($transfer =~ /v$/ && !(-e "$datafolder/Latin/" . subdirname('Sancti', $version) . "$transfer.txt")) { #if a transferd vigil file does not exist
  ...
  if ($name && $rank) {
    ...
    if (($name !~ /(?:Die|Feria|Sabbato)/i || octava_cc($winner, $version)) && ($dayname[0] !~ /Pasc[07]/i || $dayofweek == 0)) {
      ...
      $rankname = 'Dies Octavæ I. classis';
		} elsif ($version =~ /(1570|1910|Divino|1955|Bavariae)/ && $rule =~ /C10/) {			# B.M.V. Sabbato
      $rankname = 'Simplex';
